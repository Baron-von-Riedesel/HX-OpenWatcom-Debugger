...
ReadXMM_ proc near
                ...
                sub sp,512      ;size of FXSAVE buffer
                and sp,0FFF0h   ;stack must be para-aligned
                mov bx, sp
                .686
                .xmm
                fxsave ss:[bx]
                lea si, [bx+32+8*16] ;locate start of XMM regs
                mov es, dx
                mov di, ax
                push ds
                push ss
...
WriteXMM_ proc near
                ...
                .686
                .xmm
                fxsave ss:[bx]  ;init buffer with current values
                push ds
                mov ds, dx
                mov si, ax
                push ss
                pop es
                lea di,[bx+32+8*16] ;locate start of XMM regs
                mov cx, 8*16 / 4
                rep movsd
                lodsd
                mov ss:[bx+24], eax ;mxcsr
                pop ds
                fxrstor ss:[bx]  ;and finally write new values
