...
extern void ReadXMM( struct x86_xmm * );   /* japheth */
extern void WriteXMM( struct x86_xmm * );  /* japheth */
...
unsigned ReqRead_regs( void )
{
    ...
    if( Have87Emu() ) {
        ...
        Read8087( &mr->x86.fpu );
        if ( Flags.IsXMM ) {                /* japheth */
            //memset( &mr->x86.xmm, 0x42, sizeof( mr->x86.xmm ) );
            ReadXMM( &mr->x86.xmm );
        }
    } else {
...
unsigned ReqProg_load( void )
{
    ...
    TaskRegs.EFL |= FLG_I;     /* japheth: enable interrupt flag */
