...
unsigned ExecProg( bool tracing, bool do_flip, bool want_wps )
{
	...
	DebugMsg("WD, ExecProg( tracing=%u, do_flip=%u, want_wps=%u ): enter\r\n", tracing, do_flip, want_wps );
    if( !want_wps ) ++InCall;
    ...
    for( ;; ) {
        ...
        if( how == MTRH_BREAK ) {
            if( have_brk_at_ip ) {
				DebugMsgC("WD, ExecProg(): how=MTRH_BREAK && break at CS:EIP - changing to ES_STEP_ONE\r\n" );
                es = ES_STEP_ONE;
                RemoveBPs();
		...
		DebugMsg("WD, ExecProg(): es=%X how=%X have_brk_at_ip=%u\r\n", es, how, have_brk_at_ip );
		...
		DebugMsg("WD, ExecProg(): conditions=%X run_conditions=%X\r\n", conditions, run_conditions );
        ...
        if( tracing ) {
            DebugMsgC("WD, ExecProg(): calling TraceCheck()\r\n" );
            conditions = TraceCheck( conditions );
            DebugMsg("WD, ExecProg(): conditions=%X\r\n", conditions );
        }
		...
		DebugMsg("WD, ExecProg(): will continue, conditions=%X\r\n", conditions );
...
bool ReportTrap( unsigned conditions, bool stack_cmds )
{
...
#if 0 /* japheth: no annoying MessageBox()! */
    ...
    if( conditions & COND_USER ) {
        ...
        DUIInfoBox( TxtBuff );
	} else if( conditions & COND_EXCEPTION ) {  /* japheth: simple status text for exceptions */
		RecordMsgText( &conditions ); // get the 'access violation, etc' message
		p = StrCopy( LIT( Task_Exception ), TxtBuff );
		if( MsgText != NULL ) StrCopy( MsgText, p );
