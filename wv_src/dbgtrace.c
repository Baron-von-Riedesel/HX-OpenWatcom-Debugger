...
unsigned TraceCheck( unsigned conditions )
{
    ...
    if( DbgTmpBrk.status.b.hit && (conditions & COND_STOPPERS) ) {
		DebugMsgC( "WD, TraceCheck: DgbTmpBrk hit \r\n" );
        DbgTmpBrk.status.b.active = FALSE;
	...
	DebugMsgC( "WD, TraceCheck: MS 1\r\n" );
	if( DbgTmpBrk.status.b.hit && TraceState.type == TRACE_OVER ) {
		/* japheth: no check for recursion in assembly level - it won't work */
        ...
        if( _IsOn( SW_RECURSE_CHECK ) && recursed && ( TraceState.doing_call || TraceState.unwinding ) ) {
			DebugMsg( "WD, TraceCheck: MS 1.1, recursed=%u doing_call=%u unwinding=%u\r\n", recursed, TraceState.doing_call, TraceState.unwinding );
        ...
        if( (conditions & COND_THREAD)
			...
			DebugMsgC( "WD, TraceCheck: MS 1.2\r\n" );
	...
	DebugMsgC( "WD, TraceCheck: MS 3\r\n" );
	if( CheckTraceSourceStop( &have_source ) ) {
		DebugMsgC( "WD, TraceCheck: CheckTraceSourceStop returned TRUE\r\n" );
        if( CheckForDLLThunk() ) return( conditions & ~COND_STOPPERS );
        return( conditions | COND_TRACE );
    }
	DebugMsgC( "WD, TraceCheck: tracing source\r\n" );
    /*
